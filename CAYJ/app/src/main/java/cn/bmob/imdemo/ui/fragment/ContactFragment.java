    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        IMutlipleItem<Friend> mutlipleItem = new IMutlipleItem<Friend>() {
            ...
            public int getItemViewType(int postion, Friend friend) {
                if (postion == 0) {
                    return ContactAdapter.TYPE_NEW_FRIEND; //0
                } else {
                    return ContactAdapter.TYPE_ITEM;  //1
                }
            }
        ...
        adapter.setOnRecyclerViewListener(new OnRecyclerViewListener() {
            ...
            public void onItemClick(int position) {
                if (position == 0) {//跳转到新朋友页面
                    ...
                    BmobIMUserInfo info = new BmobIMUserInfo(user.getObjectId(),
                            user.getUsername(), user.getAvatar());
                    //启动一个会话，实际上就是在本地数据库的会话列表中先创建（如果没有）
