...
public class NewFriendAdapter extends BaseRecyclerAdapter<NewFriend>{
    ...
    public NewFriendAdapter(Context context, IMutlipleItem<NewFriend> items,
                            Collection<NewFriend> datas) {
        super(context,items,datas);
    }

    @Override
    public void bindView(final BaseRecyclerHolder holder, final NewFriend add, int position) {
        holder.setImageView(add == null ? null : add.getAvatar(),
                R.mipmap.head, R.id.iv_recent_avatar);
        ...
        if(status==null || status== Config.STATUS_VERIFY_NONE||status ==Config.STATUS_VERIFY_READED){
            ...
            holder.setOnClickListener(R.id.btn_aggree,
    ...
    private void sendAgreeAddFriendMessage(final NewFriend add,final SaveListener listener){
        ...
        //如果为true,则表明为暂态会话，也就是说该会话仅执行发送消息的操作，
        // 不会保存会话和消息到本地数据库中
        BmobIMConversation c = BmobIM.getInstance().startPrivateConversation(info,true,null);
        //这个obtain方法才是真正创建一个管理消息发送的会话
        BmobIMConversation conversation = BmobIMConversation.obtain(BmobIMClient.getInstance(),c);
        //而AgreeAddFriendMessage的isTransient设置为false，
        // 表明我希望在对方的会话数据库中保存该类型的消息
        AgreeAddFriendMessage msg =new AgreeAddFriendMessage();
        User currentUser = BmobUser.getCurrentUser(context, User.class);
        msg.setContent("我通过了你的好友验证请求，你现在可以套路我了!");
        //---这句话是直接存储到对方的消息表中的
        Map<String,Object> map =new HashMap<>();
        map.put("msg",currentUser.getUsername()+"已同意添加你为好友");//显示在通知栏上面的内容
        map.put("uid",currentUser.getObjectId());//发送者的uid-方便请求添加的发送方找到该条添加好友的请求
