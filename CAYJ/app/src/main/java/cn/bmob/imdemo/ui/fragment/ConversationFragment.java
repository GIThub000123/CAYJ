...
/**会话界面
 * @author :smile
 * setListener 先加载数据在startActivity
 * 加载数据和加载adapter先后不影响
 * 加载ObserverViewTree后即可加载数据和adapter
    ...
    private void setListener(){
        rootView.getViewTreeObserver().addOnGlobalLayoutListener(
            ...
            public void onGlobalLayout() {
                ...
                sw_refresh.setRefreshing(true);   //setRefreshing正在刷新
                query();
            }
        });
...
//        adapter.bindDatas(BmobIM.getInstance().loadAllConversation());
    ...
    private List<Conversation> getConversations(){
        ...
        List<NewFriend> friends = NewFriendManager.getInstance(getActivity()).getAllNewFriendByTime();
